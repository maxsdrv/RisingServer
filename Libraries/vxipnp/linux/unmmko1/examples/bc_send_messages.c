...
int main() {
    ...
    commands[3] = unmmko1_f4 (UNMMKO1_BUS_B, RT_1, SA_MC0, 2);
    ...
    commands[9] = unmmko1_f10(UNMMKO1_BUS_B, SA_MC31, 17, 0);
    ...
    for (command_index = 0; command_index < 10; ++command_index) {
        unmkocheck(unmmko1_bc_transmit_command(session, commands[command_index]));
        Sleep(1);
    }
